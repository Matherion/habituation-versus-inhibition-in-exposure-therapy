...
- Cram??r's V (used for cross tables)
...
```{r extraction-script-importing, fig.width=22, fig.height=8}
...
tidyTreatments$cleanIngredient <-
  dplyr::case_when(tidyTreatments$ingredient == "fearHierarchy" ~ "Angsthi??rarchie",
                   tidyTreatments$ingredient == "fearMon" ~ "Angstmonitoring",
                   tidyTreatments$ingredient == "lvlBasedFear" ~ "Einde: angstniveau",
                   tidyTreatments$ingredient == "lvlBasedExpect" ~ "Einde: verwachtingsdisconfirmatie",
                   tidyTreatments$ingredient == "expectState" ~ "Expliciteren verwachtingen",
                   tidyTreatments$ingredient == "expectMonitor" ~ "Monitoring verwachtingen",
                   tidyTreatments$ingredient == "learnEval" ~ "Leer-evaluatie",
                   tidyTreatments$ingredient == "expBasedFear" ~ "Einde: voorwaarden (angst)",
                   tidyTreatments$ingredient == "expBasedExpect" ~ "Einde: voorwaarden (verwachtingen)",
                   tidyTreatments$ingredient == "expBasedNonspec" ~ "Einde: voorwaarden (niet specifiek)",
                   tidyTreatments$ingredient == "lessOverest" ~ "Overschatting verminderen",
                   tidyTreatments$ingredient == "deepExt" ~ "Deepened extinction",
                   tidyTreatments$ingredient == "pharma" ~ "Farmacologische middelen",
                   tidyTreatments$ingredient == "occReinfExt" ~ "Occasioneel bekrachtigde extinctie",
                   tidyTreatments$ingredient == "removSaf" ~ "Veiligheidsgedragingen verhinderen",
                   tidyTreatments$ingredient == "variab" ~ "Variabiliteit",
                   tidyTreatments$ingredient == "retrCues" ~ "Geheugensteuntjes",
                   tidyTreatments$ingredient == "multiCont" ~ "Multipele contexten",
                   tidyTreatments$ingredient == "preExposure" ~ "Pre-exposure",
                   tidyTreatments$ingredient == "contingency" ~ "Beloningen",
                   TRUE ~ "Unknown");
...
tidyTreatments$supportsLabel <-
  factor(tidyTreatments$supportsLabel,
         levels=1:3,
         labels=c("Habituatie", "Inhibitie", "Neutraal"),
         ordered=TRUE);

tidyTreatments$Aanwezigheid <-
  dplyr::case_when(tidyTreatments$presence == 'present' ~ 'Aanwezig',
                   tidyTreatments$presence == 'absent' ~ 'Afwezig',
                   tidyTreatments$presence == 'unknown' ~ 'Onbekend',
                   tidyTreatments$presence == 'not coded' ~ 'Niet gecodeerd')
  ...
  res <-
    ggplot2::ggplot(data=data,
                    mapping=ggplot2::aes(x=study,
                                         ...
                                         alpha=supportsLabel)) +
    ggplot2::geom_raster() +
    ggplot2::scale_fill_viridis_d(name="Aanwezigheid") +
    ggplot2::scale_color_viridis_d(name="Aanwezigheid") +
    ggplot2::scale_alpha_ordinal(name="Ondersteunde theorie",
                                 range=c(1, .5)) +
    ...
    ggplot2::theme(panel.grid = ggplot2::element_blank(),
                   ...
                   legend.box.background = ggplot2::element_rect(fill = "transparent",
                                                                 color = "transparent")) +
    ggplot2::labs(title = "Exposure ingredi??nten",
                  x = "Studie",
                  y = "Ingredi??nt");
